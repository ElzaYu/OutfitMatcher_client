...
export default function NewShoesPage() {
  ...
  async function handleShoes(event) {
    event.preventDefault();

    const errors = {};
    if (!shoesDetails.category || shoesDetails.category.trim() === "") {
      errors.category = true;
    }
    if (!shoesDetails.brand || shoesDetails.brand.trim() === "") {
      errors.brand = true;
    }
    if (!shoesDetails.color || shoesDetails.color.trim() === "") {
      errors.color = true;
    }
    if (!shoesDetails.shoes_image || shoesDetails.shoes_image.trim() === "") {
      errors.shoes_image = true;
    }
  ...
  const handleUpload = () => {
    ...
    formData.append("shoes_image", selectedFile);
    axios
  ...
  return (
    <section className="warehouse">
      <form className="warehouse__main" onSubmit={handleShoes}>
        <div className="warehouse__subcontainer">
          <div className="warehouse__wrap-address">
            <h2 className="warehouse__wrap-title">Shoes Details</h2>
            ...
            <div className="warehouse__field">
              <label className="warehouse__label">
                ...
                <input
                  className={`warehouse__input ${
                    hasError.shoes_image ? "input__error" : ""
                  }`}
                  name="shoes_image"
                  placeholder="JPG/JPEG link of an image"
                  value={shoesDetails.shoes_image}
                  onChange={handleChange}
                />
                <span className="warehouse__input-error">
                  <img src={errorIcon} alt="!" />
        ...
        <div className="warehouse__button">
          ...
          <Button name="+ Add Shoes" type="submit" />
        </div>
      </form>
    </section>
  );
